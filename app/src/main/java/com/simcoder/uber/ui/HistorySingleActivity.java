...
public class HistorySingleActivity extends AppCompatActivity
    ...
    private TextView rideLocationTextView, rideDistanceTextView,
            rideDateTextView, userNameTextView, userPhoneTextView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rideLocationTextView = findViewById(R.id.ride_location);
        rideDistanceTextView = findViewById(R.id.ride_distance);
        rideDateTextView = findViewById(R.id.ride_date);
                ...
                if (dataSnapshot.exists()) {
                    for (DataSnapshot child : dataSnapshot.getChildren()) {
                        ...
                        if (child.getKey().equals("timestamp")) {
                            rideDateTextView.setText(getDate(Long.valueOf(child.getValue().toString())));
                        }
                        ...
                        if (child.getKey().equals("distance")) {
                            ...
                            rideDistanceTextView.setText(distance.substring(0, Math.min(distance.length(), 5)) + " km");
                            //ridePrice = Double.valueOf(distance) * 0.5;

                        }
                        if (child.getKey().equals("destination")) {
                            rideLocationTextView.setText(child.getValue().toString());
                        }
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    if (map.get("phone") != null) {
                        userPhoneTextView.setText(map.get("phone").toString());
                    }
                    if (map.get("profileImageUrl") != null) {
                        Glide.with(getApplication()).load(map.get("profileImageUrl").toString()).into(userImageView);
                    }
                }
    ...
    public void onRoutingFailure(RouteException e) {
        if (e != null) {
            Toast.makeText(this, "Error Fail Draw road: " + e.getMessage(), Toast.LENGTH_LONG).show();
        } else {
